...
static void NUM_BLUE_timer(void *priv)
{
   ...
   // }
    ...
    n.type = TYPE_NUM;
    n.numbs = 1;
    n.number[0] = step_not_clear;//this->steps;
    ui_number_update_by_id(NUM_BLUE, &n);//刷新
    percent = /*this->steps*/step_not_clear * 100 / this->target_steps;
...
static void NUM_GREEN_timer(void *priv)
{
    ...
    ui_io_set(IO_FRAME, HIGH);
	percent = step_not_clear;
	printf("================================percent = %d last_percent = %d",percent,last_percent);
	if (last_percent != percent) {
	second++;
	last_percent = percent;
	}
    ...
    if (min>30/*this->min > 30*/) {
        min = 0;/*this->min = 0;*/
    }
	printf("=============================second = %d min = %d,percent = %d last_percent = %d",second,min,percent,last_percent);
    n.type = TYPE_NUM;
    n.numbs = 1;
    n.number[0] = min;//this->min;
    ui_number_update_by_id(NUM_GREEN_1, &n);
    n.number[0] = this->target_min;
    ui_number_update_by_id(NUM_GREEN_2, &n);
    percent = /*this->min*/min * 100 / this->target_min;
   // if (last_percent != percent) {
...
const u8 icon_48x49_rgb565[4704] = {//format:r5g3 g3b6
...
const u8 icon_48x49_argb8565[7056] = {//format:a8 r5g3 g3b6
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
void draw_rgb565_on_display_buffer(struct draw_context *dc, u8 *buf, int x, int y, int width, int height)
{
    struct rect rect;
    struct rect r;
    int w, h;
    ...
    if (get_rect_cover(&dc->draw, &rect, &r)) {
        for (h = 0; h < r.height; h++) {
            memcpy(&dc->buf[(r.top + h - dc->disp.top) * dc->disp.width * 2 + (r.left - dc->disp.left) * 2], &buf[(r.top + h - rect.top)*width * 2 + (r.left - rect.left) * 2], r.width * 2);
        }
    }
}

void draw_argb8565_on_display_buffer(struct draw_context *dc, u8 *buf, int x, int y, int width, int height)
{
    struct rect rect;
    struct rect r;
    int w, h;
    ...
    if (get_rect_cover(&dc->draw, &rect, &r)) {
        for (h = 0; h < r.height; h++) {
            for (w = 0; w < r.width; w++) {
                alpha = buf[(r.top + h - rect.top) * width * 3 + (r.left + w - rect.left) * 3];
                forecolor = (buf[(r.top + h - rect.top) * width * 3 + (r.left + w - rect.left) * 3 + 1] << 8)  | buf[(r.top + h - rect.top) * width * 3 + (r.left + w - rect.left) * 3 + 2];
                if (alpha) {
                    offset = (r.top + h - dc->disp.top) * dc->disp.width + (r.left + w - dc->disp.left);
                    if ((offset * 2 + 1) < dc->len) {
                        backcolor = pdisp[offset];
                        mixedcolor = ui_draw_get_mixed_pixel((backcolor >> 8) | ((backcolor & 0xff) << 8), forecolor, alpha);
                        pdisp[offset] = mixedcolor;
                    }
                }
            }
        }
...
static int DRAW_DEMO_onchange(void *_ctrl, enum element_change_event event, void *arg)
{
    ...
    struct draw_context *dc = (struct draw_context *)arg;
    const u32 color_tab[] = {0xf800, 0x07c0, 0x003f};
    static int color_index = 0;
    struct rect rect = {0};
    struct rect r = {0};
    int w, h;
    int angle;
    u8 text[] = {0xD7, 0xD6, 0xBF, 0xE2, 0xCF, 0xD4, 0xCA, 0xBE};
    ...
    case ON_CHANGE_SHOW_POST:
        ...
        for (h = 0; h < 3; h++) {
            for (w = 0; w < 3; w++) {
                ui_fill_rect(dc, rect.left + 35 * w, rect.top + 35 * h, 30, 30, 0xf800);
            }
        }
#endif

        //显示图片
        /* ui_draw_image(dc, 38, PAGE38_95ad_WATCH_SELECT, rect.left + 16, rect.top + 16); */
        /* ui_draw_image(dc, 38, PAGE38_9304_WATCH1, rect.left + 0, rect.top + 0); */
        /* ui_draw_image(dc, 1, PAGE1_7519_BNUM0, rect.left + 100, rect.top + 200); */
        /* ui_draw_image(dc, 1, PAGE1_1a5c_BNUM1, rect.left + 125, rect.top + 200); */
        /* ui_draw_image(dc, 1, PAGE1_ab93_BNUM2, rect.left + 150, rect.top + 200); */
#if 0
        //显示文字
        ui_draw_ascii(dc, "01234567", 8, rect.left, rect.top + 50, 0xffff);
        ui_draw_ascii(dc, "abcdefgh", 8, rect.left, rect.top + 66, 0xffff);
        ui_draw_text(dc, FONT_ENCODE_ANSI, 0, (char *)text, sizeof(text), rect.left, rect.top + 82, 0x07c0);
        ui_draw_strpic(dc, M26, rect.left, rect.top + 120, 0x003f);

#endif
        //画矩形
        ui_draw_rect(dc, rect.left + 0, rect.top + 0, 200, 200, 0xffff);

        //画线
        ui_draw_line(dc, rect.left, rect.top, rect.left + rect.width, rect.top + rect.height, 0xf800);
        ui_draw_line(dc, rect.left, rect.top + rect.height, rect.left + rect.width, rect.top, 0xf800);

        for (angle = 0; angle < 360; angle += 6) {
            ui_draw_line_by_angle(dc, rect.left + 100, rect.top + 100, 100, angle, 0xffff);
        }
...
#if 0
        ui_draw_circle(dc, rect.left + 100, rect.top + 100, 100, 100, 225, 225 + 360, 0x07c0, 75);

        //画圆(顺序针方向,12点钟方向为0度)
        //在显存里画图
        draw_rgb565_on_display_buffer(dc, icon_48x49_rgb565, rect.left + 100, rect.top + 100, 48, 49);//不带alpha的显示
        draw_argb8565_on_display_buffer(dc, icon_48x49_argb8565, rect.left + 100 + 50, rect.top + 100, 48, 49);//带alpha的显示
#endif
        break;

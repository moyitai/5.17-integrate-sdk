...
u32 sport_data_func_attr_heart_rate_get(void *priv, u8 attr, u8 sub_mask, u8 *buf, u16 buf_size, u32 offset)
{
    ...
    if (sub_mask & BIT(SPORT_DATA_REAL_TIME_HEART_RATE)) {
        heart_rate_data[data_len++] = 68;//__heartrate_hd.heart_rate_data_get(LAST_DATA, NULL);
#if TCFG_HR_SENSOR_ENABLE
printf("get in heart for value eleven");
        sys_timeout_add(NULL, read_heart_rate_task, 1000);
		printf(" get_hrs_results = %d",get_hrs_results());
		if(!get_hrs_results()){
		heart_rate_data[data_len++] = get_hrs_results();
			}
#endif
    }
	printf("LAST_DATA---done");

    if (sub_mask & BIT(SPORT_DATA_RESTING_HEART_RATE)) {
		printf("SPORT_DATA_RESTING_HEART_RATE=====");
        heart_rate_data[data_len++]	= 70;//__heartrate_hd.heart_rate_data_get(MIN_DATA, NULL);
       
    }
		printf("MIN_DATA---done");

    if (sub_mask & BIT(SPORT_DATA_MAX_HEART_RATE)) {
		printf("SPORT_DATA_MAX_HEART_RATE=====1111111");
        heart_rate_data[data_len++]	= 75;//__heartrate_hd.heart_rate_data_get(MAX_DATA, NULL);
    }
	printf("MAX_DATA---done");

    if (0xff == sub_mask) {
		printf("clear rate data ==================");
	...
	printf("sub_mask---done");

    rlen = add_one_attr_with_submask_huge(buf, buf_size, offset, attr, sub_mask, heart_rate_data, data_len);
	printf("add_one_attr_with_submask_huge---done %d %d %d",heart_rate_data[0],heart_rate_data[1],heart_rate_data[2]);
	...
	printf("sport_data_global_var_deal---done");
    return rlen;
}

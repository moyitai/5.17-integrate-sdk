...
int JL_rcsp_event_handler(struct rcsp_event *rcsp)
{
    ...
    case  MSG_JL_UPDATE_MUSIC_PLAYER_TIME_TEMER:
		printf("%s MSG_JL_UPDATE_MUSIC_PLAYER_TIME_TEMER\n",__func__);
        if (JL_rcsp_get_auth_flag()) {
...
static void smartbox_rcsp_disconnect(void)
{
    ...
    smartbox_extra_flash_disconnect_tips(10);
#if RCSP_UPDATE_EN
    rcsp_resume();
#endif
    sport_data_func_release();
    smartbox_timer_contrl(0);
}

static void smartbox_rcsp_connect(void)
{
	printf("%s",__func__);
    smartbox_extra_flash_disconnect_tips(0);
    sport_data_func_init();
    smartbox_timer_contrl(1);
}

static void smartbox_user_state_event(u8 opcode, u8 state)
{
	printf("%s %d %d\n",__func__,opcode,state);
    u8 data[2] = {0};
    data[0] = opcode;
    data[1] = state;
...
{
	printf("%s---ble_status = %d------[two]",__func__,ble_status);
...
{
	printf("%s-----spp_status = %d------[two]",__func__,spp_status);
...
static void smartbox_user_state_handler(u8 *param, u8 param_len)
{
printf("%s-param[0] = %d------[one]",__func__,param[0]);
    u8 opcode = param[0];
    u8 state = param[1];
    switch (opcode) {
...
{
	printf("%s-opcode = %d------[one]",__func__,opcode);

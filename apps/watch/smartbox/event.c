...
static void smartbox_common_event_deal(int msg, int argc, int *argv)
{
...
#if (TCFG_APP_RTC_EN && !SMARTBOX_APP_RTC_EN)
    ...
    case USER_MSG_SMARTBOX_RTC_UPDATE_STATE:
        ...
        break;
    case USER_MSG_SMARTBOX_CONNECT_RESET:
        printf("USER_MSG_SMARTBOX_CONNECT_RESET\n");
        extern void file_transfer_close(void);
        file_transfer_close();
        break;
    case USER_MSG_SMARTBOX_FILE_TRANS_BACK:
        printf("USER_MSG_SMARTBOX_FILE_TRANS_BACK\n");
        extern void file_trans_back_close(void);
        file_trans_back_close();
        break;
    case USER_MSG_SMARTBOX_NFC_FILE_TRANS_BACK:
        printf("USER_MSG_SMARTBOX_NFC_FILE_TRANS_BACK\n");
        extern void nfc_file_trans_back_end(void *priv, u32 handler, u8 op, int result, int param);
        nfc_file_trans_back_end((void *)smart, (u32)argv[1], (u8)argv[2], argv[3], argv[4]);
        break;
    case USER_MSG_SMARTBOX_SPORT_DATA_EVENT:
        printf("USER_MSG_SMARTBOX_SPORT_DATA_EVENT\n");
        extern void sport_data_func_event(void *priv, u8 flag);
        sport_data_func_event((void *)smart, (u8)argv[1]);
        break;
    default:
        break;
    }
...
int smartbox_bt_key_event_deal(u8 key_event, int ret)
{
...
int smartbox_common_key_event_deal(u8 key_event, int ret)
{
    struct smartbox *smart = smartbox_handle_get();
    if (smart == NULL) {
        return ret;
    ...
    case  KEY_VOL_UP:
        ...
        break;
    default:
        if (smartbox_misc_event_deal(key_event, NULL)) {
            break;
        }
        break;
    }

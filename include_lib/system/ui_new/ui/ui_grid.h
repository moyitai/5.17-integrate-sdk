...
int ui_grid_set_energy(struct ui_grid *grid, float a, float val);//列表惯性滚动的负加速度值以及和滑动速度相关的参数，在init的时候可以设置,a默认值是-0.010,val默认值是0.3
int grid_energy_auto_center(struct ui_grid *grid, u8 auto_center_enable);//是否启动滑动居中

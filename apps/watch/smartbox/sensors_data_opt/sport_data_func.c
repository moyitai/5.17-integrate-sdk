...
#define SPORT_DATA_TIMED_NOFIFYCATION_TIME	1*60*1000	// 默认关闭连上app后定时发送运动数据
...
{
	printf("%s,OpCode = %d,OpCode_SN = %d,data = %s",__func__,OpCode,OpCode_SN,data);
...
printf("%s,OpCode = %d,OpCode_SN = %d,data = %s",__func__,OpCode,OpCode_SN,data);
    int ret = -1;
    switch (OpCode) {
    case JL_OPCODE_SPORTS_DATA_INFO_GET://A0=160
        ret = 0;
        get_sport_data_info(priv, OpCode, OpCode_SN, data, len);
        break;
    case JL_OPCODE_SPORTS_DATA_INFO_SET://A1=161

...
#if 0//TCFG_GSENOR_USER_IIC_TYPE
...
G_SENSOR_INFO  __gSensor_info = {.iic_hdl = 1,.iic_delay = 1};
...
{
	//printf("%s r_chip_id:%d register_address:%d data_len:%d \n",__func__,r_chip_id,register_address,data_len);

    spin_lock(&sensor_iic);
	//printf("spin_lock(&sensor_iic) hd1=%d;\n",gSensor_info->iic_hdl);
    u8 read_len = 0;
    iic_start(gSensor_info->iic_hdl);
...
int gravity_sensor_init(void *_data)
{
    ...
    if (gSensor_hdl->gravity_sensor_init()) {
        log_e(">>>>gSensor_Int ERROR = %d\n",gSensor_hdl->gravity_sensor_init());
    } else {

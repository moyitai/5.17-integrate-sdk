...
static u32 target_feature_ota_update_status(void *priv, u8 attr, u8 *buf, u16 buf_size, u32 offset)
{
    ...
    update_status_value[2] = smartbox_eflash_update_flag_get();
...
static u32 target_feature_file_transfer_info(void *priv, u8 attr, u8 *buf, u16 buf_size, u32 offset)
{
    ...
    if (smart->file_trans_back_mode) {
        config |= BIT(1); // 文件回传支持手机选择外挂flash还是sd卡
    }

    if (smart->file_simple_trans_mode) {
        config |= BIT(2);
    }
...
static const attr_get_func target_feature_mask_get_tab[FEATURE_ATTR_TYPE_MAX] = {
    ...
    [FEATURE_ATTR_TYPE_FILE_TRANSFER_INFO] = target_feature_file_transfer_info,
};

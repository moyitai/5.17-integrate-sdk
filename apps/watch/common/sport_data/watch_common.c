...
#else
printf("blood get in this one");
    wspo2.SPO2 = get_spo_result();//99;
    return wspo2.SPO2;
...
#if TCFG_HR_SENSOR_ENABLE
printf("get in heart for value nine");
            if ((watch_switch & BIT(i)) == 0) {
                heart_rate_stop();
            } else {
                heart_rate_start();
...
#endif
#if TCFG_HR_SENSOR_ENABLE
printf("get in heart for value ten");
    char search_hrsensor;
    hr_sensor_io_ctl(HR_SEARCH_SENSOR, &search_hrsensor);
    if (search_hrsensor == 0) {
        log_e("watch_hrsensor_not_find\n");
...
#endif
printf("blood get in this three");
#if TCFG_BMP280_ENABLE
